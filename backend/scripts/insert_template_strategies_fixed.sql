-- ============================================
-- 템플릿 전략 생성 SQL 스크립트 (수정본)
-- Supabase SQL Editor에서 직접 실행
-- 사용자 ID 포함 버전
-- ============================================

-- 사용자 ID 설정 (본인의 user_id로 변경 필요)
DO $$
DECLARE
    v_user_id UUID := 'f912da32-897f-4dbb-9242-3a438e9733a8';  -- 여기에 본인의 user_id 입력
BEGIN
    -- 임시 테이블에 저장
    CREATE TEMP TABLE IF NOT EXISTS temp_user_config (user_id UUID);
    INSERT INTO temp_user_config VALUES (v_user_id);
END $$;
...
SELECT
    ...
    'f912da32-897f-4dbb-9242-3a438e9733a8'::uuid,  -- user_id 추가
    true,
    NOW()
WHERE NOT EXISTS (
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] 볼린저밴드'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] RSI 반전'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] MACD 시그널'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] 스캘핑'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] 스윙 트레이딩'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] 복합 전략 A'
);
...
SELECT
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM strategies WHERE name = '[템플릿] 복합 전략 B'
);
