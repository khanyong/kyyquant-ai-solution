"""
키움 OpenAPI+ 전체 종목 다운로드 (10년치 완전판)
- 연속조회를 통한 10년치 일봉 데이터
- 진행률 표시
- 중단/재개 가능
"""
import sys
...
class KiwoomDownloader(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Kiwoom Data Downloader - 10 Years")
        ...
        self.current_data = []
        self.all_stocks = []
        self.progress_file = "download_progress_10y.json"
        self.data_dir = "D:/Dev/auto_stock/data/kiwoom_10years"
    ...
    def get_all_stocks(self):
        ...
        for code in kospi_list:
            ...
            if name and not any(x in name for x in ['스팩', 'ETN', '리츠']):  # ETF 포함
    ...
    def download_stock_data(self, code, name, market):
        ...
        print(f"  목표: {self.target_date} ~ {end_date.strftime('%Y%m%d')}")
        ...
        if ret == 0:
            ...
            # 연속조회
            query_count = 1
            while self.prev_next == "2" and query_count < 20:  # 최대 20번 조회 (약 12000일)
                time.sleep(0.2)  # API 제한 대응
                
                print(f"    연속조회 {query_count+1}회 (현재 {len(self.current_data)}일)")
                
                ret = self.ocx.dynamicCall("CommRqData(QString, QString, int, QString)",
                                          "일봉조회", "opt10081", 2, "0101")  # prev_next=2
                ...
                # 목표 날짜 도달 확인
                if self.current_data and self.current_data[-1]['date'] <= self.target_date:
                    break
            
            # CSV 저장
            if self.current_data:
                # 날짜 순으로 정렬 (오래된 날짜부터)
                self.current_data.sort(key=lambda x: x['date'])
                ...
                # 실제 기간 표시
                if self.current_data:
                    start = self.current_data[0]['date']
                    end = self.current_data[-1]['date']
                    print(f"  OK: {len(self.current_data)}일 저장 ({start} ~ {end})")
