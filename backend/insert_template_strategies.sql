-- ============================================================
-- 8개 템플릿 전략 생성 (최신 지표 사용, left/right 형식)
-- ============================================================

-- user_id NULL 허용 설정 (템플릿은 공용)
...
SELECT
    ...
    jsonb_array_length(config->'buyConditions') as buy_count,
    jsonb_array_length(config->'sellConditions') as sell_count,
    created_at
FROM strategies
ORDER BY created_at;
...
BEGIN
    ...
    RAISE NOTICE '  Invalid strategies: %', invalid_count;

    IF invalid_count = 0 THEN
        RAISE NOTICE '  ✅ All conditions use standard format (left/right)';
    ELSE
        RAISE NOTICE '  ⚠️  Some conditions are invalid';
    END IF;
    RAISE NOTICE '=================================';
END $$;
