...
SELECT
  ...
  CASE
    WHEN COALESCE(ARRAY_LENGTH(s.target_stocks, 1), ARRAY_LENGTH(s.universe, 1), 0) = 0
      THEN '❌ 투자 유니버스 비어있음 → 모니터링할 종목을 추가하세요'
    ELSE '✅ 모니터링 종목 ' || COALESCE(ARRAY_LENGTH(s.target_stocks, 1), ARRAY_LENGTH(s.universe, 1)) || '개'
  END as status
FROM strategies s
WHERE s.user_id = 'f912da32-897f-4dbb-9242-3a438e9733a8'
  AND s.is_active = true;

-- 4. strategy_monitoring 실행 여부
SELECT
  '=== 4. 조건 모니터링 워크플로우 ===' as section,
...
SELECT
  ...
  CASE
    ...
    WHEN NOT EXISTS (
      ...
      THEN '→ target_stocks 또는 universe에 종목 코드 추가: UPDATE strategies SET target_stocks = ARRAY[''005930'', ''000660''] WHERE id = ''전략ID'';'
...
-- 11. 자동매매 활성화 상태 확인 ⭐
SELECT
  '=== 11. 자동매매 설정 확인 ===' as section,
  s.name,
  s.is_active,
  s.auto_trade_enabled,
  s.auto_execute,
  COALESCE(ARRAY_LENGTH(s.target_stocks, 1), ARRAY_LENGTH(s.universe, 1), 0) as stock_count,
  CASE
    WHEN s.is_active = false THEN '❌ 전략 비활성화'
    WHEN s.auto_trade_enabled = false AND s.auto_execute = false THEN '❌ 자동매매 꺼짐'
    WHEN COALESCE(ARRAY_LENGTH(s.target_stocks, 1), ARRAY_LENGTH(s.universe, 1), 0) = 0 THEN '❌ 투자 유니버스 없음'
  ...
  CASE
    ...
    WHEN COALESCE(ARRAY_LENGTH(s.target_stocks, 1), ARRAY_LENGTH(s.universe, 1), 0) = 0 THEN 'UPDATE strategies SET target_stocks = ARRAY[''005930'', ''000660''] WHERE id=''' || s.id || ''';'
    ELSE '설정 정상'
  END as fix_query
FROM strategies s
WHERE s.user_id = 'f912da32-897f-4dbb-9242-3a438e9733a8'
