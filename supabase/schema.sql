...
CREATE INDEX idx_price_data_stock_date ON price_data(stock_code, date DESC);
...
CREATE INDEX idx_news_alerts_stock ON news_alerts(stock_code);
CREATE INDEX idx_news_alerts_published ON news_alerts(published_at DESC);
...
CREATE POLICY "Anyone can view realtime prices" ON realtime_prices
    FOR SELECT USING (true);

CREATE POLICY "Anyone can view market index" ON market_index
    FOR SELECT USING (true);

CREATE POLICY "Anyone can view news alerts" ON news_alerts
    FOR SELECT USING (true);
